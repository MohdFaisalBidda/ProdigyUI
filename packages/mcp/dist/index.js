  ...
  async ({ query }) => {
    try {
      ...
      const results = registry.components.filter(
        (c) => c.name.toLowerCase().includes(searchTerm) || c.title.toLowerCase().includes(searchTerm) || c.description.toLowerCase().includes(searchTerm) || c.tag.toLowerCase().includes(searchTerm)
      );
...
server.tool(
  ...
  async ({ name }) => {
    try {
      ...
      const installSteps = component.installSteps.length > 0 ? component.installSteps : [{ title: "No additional dependencies", description: "This component has no peer dependencies", code: "# No dependencies needed" }];
      return {
        content: [
          {
            type: "text",
...
function getTypicalUseCases(component) {
  const useCases = {
    "stroke-cards": [
...
function getRecommendedProps(component) {
...
function getCommonPatterns(component) {
  const patterns = {
    "stroke-cards": [
...
function getBestPractices(component) {
  return [
...
function getPerformanceConsiderations(component) {
...
function findSimilarComponents(component, allComponents, criteria) {
  return allComponents.filter((c) => c.name !== component.name).filter((c) => {
    switch (criteria) {
      case "functionality":
...
function getSimilarityReasoning(component, similar, criteria) {
  const reasons = {
...
function generateComponentVariant(component, variantType, customProps, description) {
  const variants = {
    minimal: {
      type: "minimal",
...
function generateMinimalVariant(component) {
  return `<${component.name} />`;
}
function generateEnhancedVariant(component) {
  return `<${component.name}
  className="enhanced-component"
  // Add enhanced props here
/>`;
}
function generateCustomVariant(component, customProps) {
  if (!customProps) return generateMinimalVariant(component);
  const propsString = Object.entries(customProps).map(([key, value]) => `${key}={${JSON.stringify(value)}}`).join("\n  ");
  return `<${component.name}
  ${propsString}
/>`;
}
function generateResponsiveVariant(component) {
  return `<${component.name}
  className="w-full md:w-1/2 lg:w-1/3"
  // Add responsive props
/>`;
}
function getMinimalProps(component) {
  const requiredProps = {};
  return requiredProps;
}
function getEnhancedProps(component) {
  return {};
}
function getResponsiveProps(component) {
  return {
    className: "w-full md:w-auto"
  };
}
function generatePageLayout(description, components, layout, responsive, allComponents) {
...
function suggestComponentsForLayout(description, layout, allComponents) {
...
function generateLayoutStructure(components, layout) {
  return {
...
function generateLayoutCode(components, layout, responsive) {
...
function compareComponents(comp1, comp2, aspects) {
  const comparison = {
...
function generateIntegrationCode(component, framework, features) {
  const integrations = {
    nextjs: {
...
function generateNextJSCode(component, features) {
...
function generateViteCode(component, features) {
...
function validateCompatibility(component, target, environment) {
  const compatibility = {
    component: component.name,
    target,
    compatible: true,
...
function suggestImprovements(component, focus) {
  const suggestions = {
    component: component.name,
    suggestions: []
  };
...
function generateComponentFromDescription(description, baseComponent, complexity, allComponents) {
  return {
    description,
...
function analyzeProjectUsage(projectPath, allComponents, includeUsage) {
  return {
    projectPath,
