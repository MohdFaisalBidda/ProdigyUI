...
function generateLandingPage(type, title, description, accentColor) {
  const templates = {
    saas: {
      type: "saas",
      title,
...
function generatePageCode(template) {
  const imports = template.sections.map((s) => `import ${s.component} from "prodigy-ui";`).join("\n");
  const sections = template.sections.map((s, i) => `  <${s.component} key={${i}} ${propsToString(s.props)} />`).join("\n");
  return `${imports}
