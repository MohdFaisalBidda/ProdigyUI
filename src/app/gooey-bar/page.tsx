...
interface StatusItem {
  id: string;
  icon: 'music' | 'cube' | 'cloud' | 'wifi' | 'battery' | 'clock';
  label?: string;
  value?: string;
}

interface GooeyBarProps {
...
function GooeyBar({
  ...
  const smoothOpacity = useSpring(targetOpacity, {
    stiffness: 500,
    damping: 30,
  });
  ...
  const notchPath = useTransform(smoothX, () => `
    M 0 0
    H 600
    C 560 0 530 14 500 30
    C 470 44 430 50 380 50
    H 220
    C 170 50 130 44 100 30
    C 70 14 40 0 0 0
    Z
  `);
  ...
  useEffect(() => {
    ...
    const index = statusItems.findIndex(i => i.id === activeItem);
    ...
    targetX.set(centerX);
    targetOpacity.set(1);
  }, [activeItem, targetX, targetOpacity]);

  const activeData = useMemo(
    () => statusItems.find(i => i.id === activeItem),
    [activeItem]
  );
    ...
    <div className="flex justify-center w-full pt-4">
      <div ref={containerRef} className="relative w-full">
          ...
          {statusItems.map((item, index) => (
            ...
            >
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
              >
                <IconComponent type={item.icon} className="w-5 h-5" />
              </motion.button>
            </div>
          ))}
        </div>
        ...
        >
          ...
          >
            <motion.path d={notchPath} fill="black" />
          </svg>
            ...
            <AnimatePresence mode="wait">
              {activeData && (
                <motion.div
                  ...
                  className={`text-xs font-medium text-white ${contentClassName}`}
                >
                  {renderContent
                    ? renderContent(activeData)
                    : activeData.value || activeData.label}
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
export default function GooeyStatusBar(props: GooeyBarProps) {
  return <GooeyBar {...props} />;
}
